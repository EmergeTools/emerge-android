...
abstract class PackageSnapshotArtifacts : DefaultTask() {
  ...
  fun execute() {
    ...
    if (snapshotConfigFile.isPresent) {
      snapshotConfigFile.asFile.get().copyTo(outputDirectory.get().asFile.resolve(snapshotConfigFile.asFile.get().name), overwrite = true)
    }
    ...
    val metadata =
      ArtifactMetadata(
        ...
        snapshotConfigFile = if (snapshotConfigFile.isPresent) { snapshotConfigFile.asFile.get().name } else { null },
      )
